        ...
        <div class="col-lg-8 mrb20">
            @if (count($post_no_perpage))
                <div class="clearfix">
                        ...
                        <section class="row">
                            <div class="col-md-4">
                                <div class="content-first">
                                    <h1>
                                        <a class="first-link" href="{{route('news.detail_news',$post_no_perpage[0]->slug)}}" title="{{$post_no_perpage[0]->name}}">
                                            {{$post_no_perpage[0]->name}}
                                        </a>
                                    </h1>
                                </div>
                            </div>
                            <div class="col-md-8">
                                <figure class="mask">
                                    <a class="center-block shine-hover w-100" target="_self" href="{{route('news.detail_news',$post_no_perpage[0]->slug)}}" title="{{$post_no_perpage[0]->name}}">
                                        <img class="img-fluid center-block w-100" src="{{ RvMedia::getImageUrl($post_no_perpage[0]->image,'news_focus', false, RvMedia::getDefaultImage()) }}" alt="{{$post_no_perpage[0]->name}}">
                                    </a>
                                </figure>
                            </div>
                        </section>
                    ...
                    @php $i = 0; @endphp
                    @forelse ($post_no_perpage as $item)
                        @if($i != 0)
                                ...
                                <div class="new-home-img">
                                    <figure class="mask w-100">
                                        <a href="{{route('news.detail_news',$item->slug)}}" class="center-block shine-hover w-100" target="_self" title="{{$item->name}}">
                                            <img class="img-fluid w-100" src="{{RvMedia::getImageUrl($item->image,'image_new', false, RvMedia::getDefaultImage())}}" alt="{{$item->name}}">
                                        </a>
                                    </figure>
                                </div>
                    ...
                    <section class="row">
                        ...
                        <div class="col-lg-6 mrb20 content-area">
                            <div class="new-image w-100 shine-hover">
                                <a href="{{ route('news.detail_news', $post['slugable']->key) }}">
                            ...
                            <div class="new-content">
                                <h2>
                                    <a class="new-content__name" href="{{ route('news.detail_news', $post['slugable']->key) }}" title="{{$post->name}}">{{$post->name}}</a>
                                </h2>
                                <div class="new-content-des">
                                    {{$post->description}}
                                </div>
                                <a href="{{ route('news.detail_news', $post['slugable']->key) }}" class="new-content-readmore" title="{{$post->name}}">
                                   {{__("Xem thêm")}}
                                    <i class="fas fa-angle-right"></i>
                                </a>
                            </div>
