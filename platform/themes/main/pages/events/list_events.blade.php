            ...
            <div class="form-inline">
                <div class="col-md-9 hidden-sm-down">
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-sub">
                            <a href="{{route('events.category', $slug->key)}}">
                                <i class="fas fa-angle-right"></i>
                                {{$category->name}}
                            </a>
                        </li>
...
<section class="container">
    ...
    <div class="col-lg-8 mrb20">
        @if (count($all_posts_events))
            <div class="clearfix">
                    ...
                    <section class="row">
                        <div class="col-md-4">
                            <div class="content-first">
                                <h1>
                                    <a class="first-link" href="{{route('events.detail',$all_posts_events[0]->slug)}}" title="{{$all_posts_events[0]->name}}">
                                        {{$all_posts_events[0]->name}}
                                    </a>
                                </h1>
                            </div>
                        </div>
                        <div class="col-md-8">
                            <figure class="mask">
                                <a class="center-block shine-hover w-100" target="_self" href="{{route('events.detail',$all_posts_events[0]->slug)}}" title="{{$all_posts_events[0]->name}}">
                                    <img class="img-fluid center-block w-100" src="{{ RvMedia::getImageUrl($all_posts_events[0]->image,'news_focus', false, RvMedia::getImageUrl(theme_option('default_image'), 'news_focus')) }}" alt="{{$all_posts_events[0]->name}}">
                                </a>
                            </figure>
                        </div>
                    </section>
                ...
                @php $i = 0; @endphp
                @forelse ($all_posts_events as $item)
                    @if($i != 0)
    ...
    <div class="col-lg-4 mrb20">
        ...
        @if (count($featured_post_events))
            <div class="clearfix">
                @forelse($featured_post_events as $events)
