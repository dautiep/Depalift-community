        ...
        <div class="col-lg-8 mrb20">
            ...
            @forelse ($child as $item)
                <div class="clearfix events-mrt">
                    <h2 class="title-line">
                        <a class="title_cat" href="{{route('events.category', $item->slug)}}">{{$item->name}}</a>
                    </h2>
                    <!-- List post -->
                    <section class="row">
                        @php
                            $i=0;
                        @endphp
                        @forelse ($item['posts_events'] as $contents)
                            <div class="col-lg-6 mrb20 content-area">
                                <div class="new-image w-100 shine-hover">
                                    <a href="{{route('events.detail', $contents->slug)}}">
                                        <img src="{{ RvMedia::getImageUrl($contents->image,'featured', false, RvMedia::getImageUrl(theme_option('default_image'), 'featured')) }}" alt="" class="img-fluid center-block">
                                    </a>
                                </div>
                                <div class="new-content">
                                    <h2>
                                        <a class="new-content__name" href="{{route('events.detail', $contents->slug)}}" title="{{$contents->name}}">{{$contents->name}}</a>
                                    </h2>
                                    <div class="new-content-des">
                                        {{Str::words($contents->description, '25')}}
                                    </div>
                                    <a href="{{route('events.detail', $contents->slug)}}" class="new-content-readmore" title="{{$contents->name}}">
                                    {{__(" Xem thêm")}}
                                        <i class="fas fa-angle-right"></i>
                                    </a>
                                </div>
